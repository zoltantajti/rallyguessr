...
const GameMap = ({ pos, current, max, topic, gm }) => {
    ...
    const [mapTime, setMapTime] = useState(13);
    ...
    const timeOutEvent = () => {
        ...
        setMarkedPos({ lat: 0.0, lng: 0.0 });
        setTimeout(() => { submitCoords(); }, 2000);
    }
            ...
            {(finish) && (
                <Modal className="resultModal" show={finish} aria-labelledby="contained-modal-title-vcenter" centered>
                    <Modal.Body className="text-center">
                        ...
                        XP: <b className="color-wite">{prevXP}</b> + <b className="color-yellow">{claimedXP}</b> = <b>{xp}</b><br />
